...
client.on("messageCreate", message => {
  ...
  if (seCaen) {
    message.channel.send(`🤹 Hice malabares con ${pelotas} pelotas y se me cayeron`);
  } else {
...
if (
  ...
  if (bueno) {
    ...
    if (username === 'nootnoot9070' && Math.random() < 0.02) {
      resultado = `🍑 Te tocó sabor **kako**`;
    } else {
      resultado = `🍓 Te tocó sabor **frutilla**`;
    }
  } else {
    resultado = `🧼 Te tocó sabor **jabón**`;
  }
}
...
if (message.content.toLowerCase().includes("mahito")) {
...
"Oye ahora di' qué que te ayude Yo lo que me vo' a pajea' viendo como te lo meten Ay, si",
  ...
  message.channel.send({
    ...
    files: [img]
	  ...
	  if (
  message.content.toLowerCase().includes("silver") &&
  message.content.toLowerCase().includes("prudencio")
) {
